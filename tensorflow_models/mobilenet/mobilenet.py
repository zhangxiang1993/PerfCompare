  ...
  with tf.variable_scope(scope) as sc:
    ...
    with slim.arg_scope([slim.convolution2d, slim.separable_convolution2d],
      ...
      with slim.arg_scope([slim.batch_norm],
                          ...
                          fused=True):
        input_image = tf.reshape(inputs, [1, 224, 224, 3])
        net = slim.convolution2d(input_image, round(32 * width_multiplier), [3, 3], stride=2, padding='SAME', scope='conv_1')
...
tf_mobilenet.default_image_size = 224
    ...
    with tf.device('/cpu:0'):
      ...
      with slim.arg_scope(mobilenet_arg_scope()):
        ...
        with tf.Session() as sess:
          ...
          # save graph to protobuf
          constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def,
                                                                     ['MobileNet/Predictions/Softmax'])
          t = time.time()
          print(sess.run(logits))
          print('time for tensorflow MobineNet: %s ms' % ((time.time() - t) * 1000))
          with tf.gfile.GFile('mobilenet_tf.pb', mode='wb') as f:
            f.write(constant_graph.SerializeToString())


if __name__ == '__main__':
  eval_tensorflow_mobilenet()