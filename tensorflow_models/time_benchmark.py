...
def time_tensorflow_run(session, target, info_string):
  ...
  for i in range(FLAGS.num_batches + num_steps_burn_in):
    ...
    logits = session.run(target)
    duration = time.time() - start_time

    if i >= num_steps_burn_in:
      if not i % 10:

        print('%s: step %d, duration = %.3f' % (datetime.now(), i - num_steps_burn_in, duration*1000))
      total_duration += duration
      total_duration_squared += duration * duration
  saver.save(session, './mobilenet_tf_forward.ckpt')
  # constant_graph = graph_util.convert_variables_to_constants(session, session.graph_def, ['op_to_store'])
  mn = total_duration / FLAGS.num_batches
  vr = total_duration_squared / FLAGS.num_batches - mn * mn
  sd = math.sqrt(vr)
  print('%s: %s across %d steps, %.3f +/- %.3f msec / batch' % (datetime.now(), info_string, FLAGS.num_batches, mn*1000, sd*1000))
...
def time_tensorflow_run_placeholder(session, target, feed_dict, info_string):
  ...
  for i in range(FLAGS.num_batches + num_steps_burn_in):
    ...
    if i >= num_steps_burn_in:
      ...
      total_duration += duration
      total_duration_squared += duration * duration
  saver.save(session, './mobilenet_tf.ckpt')
  mn = total_duration / FLAGS.num_batches
  vr = total_duration_squared / FLAGS.num_batches - mn * mn
  sd = math.sqrt(vr)
